function loadTabs() {
	const allTabContainers = document.querySelectorAll<HTMLDivElement>('.sui-tabs-container');
	...
	for (const tabContainer of allTabContainers) {
		...
		const switchTab = (target: HTMLElement, container: HTMLElement, originatedFromSync = false) => {
			...
			const newActiveTabContent = container.querySelector<HTMLElement>(
			...
			tab.addEventListener('keydown', (e) => {
				if (e.key === 'ArrowLeft' || e.key === 'ArrowRight') {
					...
					if (nextTabIndex >= 0 && nextTabIndex < tab.parentElement!.children.length) {
						...
						const lastTab = tab.parentElement!.children[
							tab.parentElement!.children.length - 1
						] as HTMLElement;
