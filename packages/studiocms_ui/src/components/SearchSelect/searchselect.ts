...
const searchSelects = document.querySelectorAll<HTMLDivElement>(
...
for (const container of searchSelects) {
  ...
  const searchWrapper = container.querySelector<HTMLDivElement>(
  ...
  searchInput.addEventListener("keyup", (e) => {
    ...
    filteredOptions = options.filter((x) =>
      x.label.includes(searchInput.value)
    );
    focusIndex = 0;
