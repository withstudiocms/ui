...
test.describe('Sidebar (single) Component', () => {
	test.beforeEach(async ({ page }) => {
		// Navigate to the test page for this component
		await page.goto('/sidebar-single-test');
	});

	test('Basic functionality', async ({ page }) => {
		await expect(page.locator('#sui-sidebar')).toBeVisible();
		await expect(page.getByRole('paragraph')).toContainText('This is some sidebar content.');
	});
	...
	[{ label: 'Basic', key: 'basic' }].forEach(({ label, key }) => {
		...
		test(`Test Accessibility - ${label} Styling (Dark Mode)`, async ({
			...
			await takeScreenshot(`Sidebar (single) - ${key} (Dark Mode)`, elmKey);
		...
		test(`Test Accessibility - ${label} Styling (Light Mode)`, async ({
			...
			await takeScreenshot(`Sidebar (single) - ${key} (Light Mode)`, elmKey);
